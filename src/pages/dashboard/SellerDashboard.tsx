...
function IncomingOrderModal({ order, onAccept, onDecline, accepting }: IncomingOrderModalProps) {
...
export default function SellerDashboard() {
    ...
    const ordersChannel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          if (newOrder.status === "confirmed" && !seenOrderIds.current.has(newOrder.id)) {
  ...
  const handleAcceptOrder = async (order: Order) => {
    ...
    toast.success(`Order #${order.order_number} accepted — now Preparing!`);
  ...
  const metricCards = [
    ...
    { label: "Pending / Confirmed", value: pendingOrders.toString(), icon: Clock, accent: "text-accent" },
                          ...
                          <TableCell colSpan={5} className="text-center py-12">
                            <div className="flex flex-col items-center">
                              ...
                              <p className="font-body text-sm text-muted-foreground max-w-xs">
                                Head to "My Products" to add your first item and start selling!
                              </p>
                            </div>
                          </TableCell>
                        </TableRow>
                ...
                <div className="flex items-center justify-between">
                  ...
                  {orders.filter((o) => o.status === "confirmed").length > 0 && (
                    <Badge className="bg-primary text-primary-foreground text-[10px] font-body gap-1">
                      ...
                      {orders.filter((o) => o.status === "confirmed").length} Incoming
                    </Badge>
                  )}
                </div>
                <CardDescription className="font-body text-sm">
              ...
              <CardContent className="space-y-2.5">
                ...
                {orders.slice(0, 8).map((order) => {
                  ...
                  return (
                    ...
                    >
                      <div className={`h-8 w-8 rounded-lg flex items-center justify-center shrink-0 ${isIncoming ? "bg-primary/15" : "bg-muted"}`}>
                        <StatusIcon className={`h-4 w-4 ${isIncoming ? "text-primary" : "text-muted-foreground"}`} />
                      </div>
                      ...
                      {isIncoming && <ChevronRight className="h-4 w-4 text-primary shrink-0" />}
                    </div>
                  );
                })}
              </CardContent>
