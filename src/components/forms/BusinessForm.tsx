/**
 * Business Form — Zod validated with loading state
 */
...
const BusinessForm = () => {
  ...
  const onSubmit = (_data: BusinessFormValues) => {
    setSubmitting(true);
    setTimeout(() => {
      toast.success("Inquiry submitted!", { description: "Our business team will contact you within 48 hours." });
      form.reset();
      setSubmitting(false);
    }, 800);
  };
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
        ...
        <Button
          type="submit"
          disabled={submitting}
          className="w-full rounded-full font-body text-sm font-semibold gap-2 bg-accent text-accent-foreground hover:bg-accent/90"
        >
